...
def is_valid_aywa_address(address, network='mainnet'):
    ...
    aywa_version = 140 if network == 'testnet' else 23
    ...
    try:
        ...
        # rescue from exception, not a valid Aywa address
        return False

    if (address_version != aywa_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.aywad_serialise())
...
def SHIM_serialise_for_aywad(sentinel_hex):
    ...
    # superblock => "trigger" in aywad
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # aywad expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    aywad_hex = serialise(obj)
    return aywad_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('aywa.conf')
