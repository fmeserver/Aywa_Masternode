...
class AywaDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some aywad methods
    ...
    @classmethod
    def from_aywa_conf(self, aywa_dot_conf):
        from aywa_config import AywaConfig
        config_text = AywaConfig.slurp_config_file(aywa_dot_conf)
        creds = AywaConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = aywalib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = aywalib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = aywalib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_aywad_proto_version_with_sentinel_ping)
