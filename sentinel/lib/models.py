...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, aywad):
        golist = aywad.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_aywad(aywad, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_aywad(self, aywad, rec):
        import decimal
        import aywalib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from aywad...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from aywad, with every run
        ...
        # get/create, then sync payment amounts, etc. from aywad - Aywad is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(aywad)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from aywad! %s" % e)
            govobj.vote_delete(aywad)
            return (govobj, None)
    ...
    def vote_delete(self, aywad):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(aywad, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, aywad, signal, outcome):
        import aywalib
        ...
        output = aywad.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = aywalib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(aywad, signal)

    def sync_network_vote(self, aywad, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = aywad.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = AYWAD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 aywa addr, non-multisig
            if not aywalib.is_valid_aywa_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Aywa address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Aywa Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (aywalib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = AYWAD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not aywalib.is_valid_aywa_address(addr, config.network):
    ...
    def hash(self):
        import aywalib
        return aywalib.hashit(self.serialise())
