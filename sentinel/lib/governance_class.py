...
class GovernanceClass(object):
    ...
    def vote(self, aywad, signal, outcome):
        return self.go.vote(aywad, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, aywad):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(aywad, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(aywad, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import aywalib
        obj_data = aywalib.SHIM_serialise_for_aywad(self.serialise())
    ...
    def aywad_serialise(self):
        import aywalib
        return aywalib.SHIM_serialise_for_aywad(self.serialise())
