...
sentinel_cfg = AywaConfig.tokenize(sentinel_config_file)
sentinel_version = "1.2.0"
min_aywad_proto_version_with_sentinel_ping = 70207


def get_aywa_conf():
    if sys.platform == 'win32':
        aywa_conf = os.path.join(os.getenv('APPDATA'), "AywaCore/aywa.conf")
    else:
        home = os.environ.get('HOME')

        aywa_conf = os.path.join(home, ".aywacore/aywa.conf")
        if sys.platform == 'darwin':
            aywa_conf = os.path.join(home, "Library/Application Support/AywaCore/aywa.conf")

    aywa_conf = sentinel_cfg.get('aywa_conf', aywa_conf)
