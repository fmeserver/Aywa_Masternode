...
@pytest.fixture
def valid_aywa_address(network='mainnet'):
    return 'AKYXSviLDMEWPxeJBYrtbA2zSat5dYwvPd' if (network == 'testnet') else 'aZwyhusRnhDT1ToBmvsUjZmJmGHuYKzRSf'


@pytest.fixture
def invalid_aywa_address(network='mainnet'):
...
def mn_status_good():
    ...
    status = {
        ...
        "service": "[2604:a880:800:a1::9b:0]:27770",
...
def test_valid_aywa_address():
    from aywalib import is_valid_aywa_address

    main = valid_aywa_address()
    test = valid_aywa_address('testnet')

    assert is_valid_aywa_address(main) is True
    assert is_valid_aywa_address(main, 'mainnet') is True
    assert is_valid_aywa_address(main, 'testnet') is False

    assert is_valid_aywa_address(test) is False
    assert is_valid_aywa_address(test, 'mainnet') is False
    assert is_valid_aywa_address(test, 'testnet') is True


def test_invalid_aywa_address():
    from aywalib import is_valid_aywa_address

    main = invalid_aywa_address()
    test = invalid_aywa_address('testnet')

    assert is_valid_aywa_address(main) is False
    assert is_valid_aywa_address(main, 'mainnet') is False
    assert is_valid_aywa_address(main, 'testnet') is False

    assert is_valid_aywa_address(test) is False
    assert is_valid_aywa_address(test, 'mainnet') is False
    assert is_valid_aywa_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from aywalib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % aywalib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(aywalib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(aywalib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(aywalib.blocks_to_seconds(16616)) == 2612035
